...
class AdminController extends CosapiController
{
  ...
  {
    $AgentOnline = AgentOnline::select(DB::raw('COUNT(*) AS count_agent'))->where('agent_name', $this->UserSystem)->first();
    $existAgent = $AgentOnline->count_agent;
    $exits = false;

    if ($existAgent != 0) {
      $exits = true;
    } else {
      $dataAgentOnline = $this->searchAgentOnline(Auth::user()->id);
      if (is_null($dataAgentOnline)) {//Crear
        (new AgentOnline())->insert([
        ...
        AgentOnline::where('agent_user_id', Auth::user()->id)->update([
          'agent_name' => Auth::user()->username,
          'agent_role' => Auth::user()->role,
          'event_id' => 11,
          'event_time' => number_format(microtime(true) * 1000, 0, '.', '')
  ...
  function searchAgentOnline($agent_user_id)
  {
    return AgentOnline::where('agent_user_id', $agent_user_id)->first();
  }
