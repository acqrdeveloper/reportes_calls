...
class UserController extends CosapiController
{
    ...
    if ($request->ajax()) {
      ...
      if($request->checkQueue){
        foreach($request->checkQueue as $keyQueue => $valueQueue){
          ...
          $dataUserQueue = $this->searchQueueAssign($request->userID,$valueQueue);
  ...
  function searchQueueAssign($user_id,$queue_id)
  {
  ...
  public function uploadPerfil(Request $request)
  {
    if ($request->ajax()) {
      if (Input::file('imgAvatar')) {
        ...
        if (Input::get('imgAvatarOriginal') == 'default_avatar.png') {
          $imageOriginal = '';
        } else {
      ...
      if (!Input::get('idProvincia') || Input::get('idProvincia') == '' || Input::get('idProvincia') == null) {
        $idProvincia = '';
      } else {
        $idProvincia = Input::get('idProvincia');
      }
      if (!Input::get('idDistrito') || Input::get('idDistrito') == '' || Input::get('idDistrito') == null) {
        $idDistrito = '';
      } else {
        $idDistrito = Input::get('idDistrito');
      }
